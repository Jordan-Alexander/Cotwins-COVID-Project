  ...
  mutate(Covid_Year_Fac = case_when(date >= ymd('2019-01-20') & date < ymd('2019-05-01')~
         ...
         Covid_Year_Fac = factor(Covid_Year_Fac,
                                 levels = c('Jan-Apr 2019',
                                            ...
                                            'Jan-Apr 2022')))
sd(dat.lmer2$numpts)
mean(dat.lmer2$numpts)

# means for 2020 and later models

dat.lmer3 = dat.lmer2 %>% filter(date >= ymd('2020-01-20'))
...
mod = lme(outcome ~ sex + relative.age + OS +
               loc.prop.miss + na.prop.miss +
               Covid_Year_Fac,
               random = ~1+Covid_Year_Fac|family/SVID,
               method = "ML", 
...
saveRDS(mod, 
        file = paste0('~/CovidLocationProject/RAship.2022/models/',
                      pheno,'_categorial_date_lme_mod.rds')
  )
