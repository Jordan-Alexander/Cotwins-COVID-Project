...
mod = lme(outcome ~ sex + relative.age + OS +
            loc.prop.miss + na.prop.miss + Covid_Year_Fac +
            weekly.cases.per.100k + Covid_Year_Fac:weekly.cases.per.100k,
          random = ~1+Covid_Year_Fac+weekly.cases.per.100k|family/SVID,
          method = "ML", 
...
saveRDS(mod, 
        file = paste0('~/CovidLocationProject/RAship.2022/models/',
                      pheno,'_casecount_year_interaction_lme_mod.rds')
)
