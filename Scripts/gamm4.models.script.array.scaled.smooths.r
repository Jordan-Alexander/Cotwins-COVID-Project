...
dat = fread('mobility.affect.covid.scaled.smoothvars.1.26.22.csv', data.table = F)
...
saveRDS(outcome.gamm, paste0('gamm4.',x,'.scaled.smooths.1.26.23.rds'))
...
if(x == 'pa.av'| x == 'na.av'){
     outcome.gamm2 <- gamm4(outcome ~ sex + age + season + wknd + pop.density+ OS +
                     ...
                     s(days.post.covid, k = 10) + s(cases.weekly.per.100k, k = 10), 
                     random = ~(1 | family/SVID),  data = dat)
     saveRDS(outcome.gamm2, paste0('gamm4.',x,'.scaled.smooths.mobility.1.26.23.rds'))}
